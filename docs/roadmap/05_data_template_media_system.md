...
>
> **⚠️ КОНФЛИКТ (F23, S8):** `ИНСТРУКЦИЯ_структура_и_ядро.md` — «карта и **закон**» (авторитет выше media-плана) —
> в §2/§5 **требует** `config/ops/*.ops.yaml` + `model_routing.yaml` + `paths.yaml`. Два документа владельца
> противоречат. Решение за владельцем (OQ6): консолидация vs декларативные ops+routing. До ответа A1/A3 не финализируем.
