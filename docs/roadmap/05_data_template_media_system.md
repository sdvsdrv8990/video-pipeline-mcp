...
**Выбор media-модели — PER-CHANNEL (решение владельца S8, эволюция замысла):** модель/голос/параметры берутся
...
> **Патерн/уникальность-слой вынесен в `10_pattern_uniqueness_system.md` (воркстрим A7, Сессия 14).**
> Листы `UNIQUENESS`/`SCENES`/`*_PATTERNS_USED`/`ASSETS_USED` (упомянутые в §1 и `video_data.schema`) —
> опора той подсистемы. Патерны конкурентов vs наши, отслеживание повторяемых слов, фоновый сигнал
> и 2 критерия уникальности сцены — там, а не здесь.
