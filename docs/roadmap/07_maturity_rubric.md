...
- **Наш уровень: L1–L2** (актуализировано S23) — data/FS-сервер работает (**68 инструментов**), локальные media-провайдеры реально исполняют (озвучка `piper`, кадры `sd-turbo`/`sdxl-turbo` на видеокарте); **A7** (патерн/уникальность) расширяет
