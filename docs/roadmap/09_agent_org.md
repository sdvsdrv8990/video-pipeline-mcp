...
> ⚠️ **Механика ОТОЗВАНА решением владельца (session 17).** `.claude/agents/*.md` (7 типов) удалены из репо:
> это агент-типы Claude Code CLI, а работа идёт через **Claude AI Web**, где они недоступны в принципе —
> 7 промптов лежали мёртвым грузом и ни одной строкой не влияли на продукт. Роли ниже читать как
> **чек-листы ответственности** (какие скилы/база знаний применимы к задаче), НЕ как спавнимых агентов.
> Живая часть документа — §4 матрица назначения задач по проходам/сессиям.
...
**Проход-L2 (Beta) — ✅ пройден к S23:** repo-гигиена · тесты в CI зелёные · **A2 распил монолита ✅** (A3 `config/ops` снят как слой) · **A6 реакции ✅** (D4/D27 закрыты). Остаток L2→L3 — T2 харнесс (F51), I5 observability, P-ось.
