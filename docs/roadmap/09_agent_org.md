...
> ⚠️ **Механика ОТОЗВАНА в S17 и ВОЗВРАЩЕНА в S25 (доп. 53) — посылка отмены перестала выполняться.**
> В S17 семь типов удалили, потому что «работа идёт через Claude AI Web, где они недоступны в
> принципе». Сегодня разработка идёт в CLI, и владелец вернул механику явно: агенты нужны **для
> разработки, а не для работы сервера**, и живут в git со всеми плюсами версии и отката.
> На диске их два (`ls .claude/agents/`), а не семь: оба **только на чтение** — разведка и разбор
> отказа. Правки и коммиты идут последовательно в главной сессии: параллельные правки в одно дерево
> дают потерянные правки, а не скорость. Роли ниже по-прежнему читать как **чек-листы
> ответственности**, а не как перечень спавнимых типов; живая часть документа — §4.
