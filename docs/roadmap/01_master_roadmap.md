...
| A7 | **Патерн/уникальность-слой** (`10_pattern_uniqueness_system.md`): патерны конкур./наши, отслеживание структуры + повторяемых слов (локальный скрипт+yaml), проверка on-demand Клодом (планировщика нет), реакции-рекомендации порядка чтения, 2 критерия уникальности сцены + сводный лист + границы сцены, конфиг-шаблоны/поиск/пороги через per-project override. Все 5 OQ решены — дизайн готов | A1′, A6, A5 | 3–4 |
...
- **Фаза 1 — Архитектура-правда:** `A1` → `A2` → `A3` → `A6` → `A7`.
  Декларативный слой + распил монолита + реакции + патерн/уникальность-слой. Закрывает «README врёт».
  `A7` идёт после `A1′`/`A6` (на их листах/реакциях стоит), продукт-наполнение меряет по мере P-оси.
