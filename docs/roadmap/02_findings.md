...
| F47 | 🟠 | **Уязвимые зависимости (pip-audit, S16 I3).** `aiohttp 3.13.5` (11 CVE, fix 3.14.1), `litellm 1.83.7` (4 CVE, fix 1.84.0 — только в provider-стабах), `python-dotenv 1.0.1` (CVE-2026-28684, fix 1.2.2). В CI pip-audit **advisory** (не бумкает гейт) — бамп прод-деп требует регрессии на живом сервере (aiohttp=транспорт), поэтому отдельным воркстримом (I5/sec) | `pip-audit` на `requirements.lock`; `.github/workflows/ci.yml` | I5/sec, dep-hardening |
