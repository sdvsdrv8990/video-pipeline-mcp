...
| F# | Sev | Находка | Пруф | → |
|---|---|---|---|---|
| F38 | 🟡 | **Мёртвый `_lock` (D28) в ОБОИХ search-классах.** `self._lock = threading.Lock()` в `__init__`, но нигде не берётся (каждый `execute_plan`/`search_parallel` заводит свой `ThreadPoolExecutor`; общего состояния под локом нет) | `fs_searcher.py:176`, `query_planner.py:113` — 0 `.acquire`/`with self._lock` | A5, code-quality |
