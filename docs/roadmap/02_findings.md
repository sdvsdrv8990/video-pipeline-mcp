...
| F52 | 🟠 | **Покрытие никогда не измерялось.** `pytest-cov` не подключён, число не посчитано ни разу → «дыры покрытия» в `03` экспертные, а не измеренные; порог в CI (п.3 плана `03`) невыполним без T0. Регрессия покрытия сейчас невидима | `grep cov` в `pyproject.toml`/`ci.yml` ∅ | **I7 T1** |
