...
> Не дефекты-факты, а **дизайн-споры**: система работает, но продуманность под вопросом. Решить до/во время подов.

- **OQ-B1 (`_SESSION_LOG.md`):** пишется в **КОРЕНЬ РЕПО** (`state_manager.py:184`, `parents[2]`), не в
  `workspace/`, и **мимо** `safe_resolve`. Три проблемы разом: (1) данные в дереве сервера — нарушение
  «двух вселенных» (DIM-15); (2) OUT6-exfil сырого `Fact.data` (F33); (3) дублирует память/аудит-трейл.
  **Спор:** нужен ли flat-`.md`-лог в корне вообще? Варианты — убрать / перенести в `workspace/` /
  заменить настоящим audit-trail (DIM-7, I5). Сейчас = известный footgun (`project-rules`).
- **OQ-B2 (A3 ops-слой vs inline-register):** `engine.register(name/title/schema/handler/group/annotations)`
  — это УЖЕ декларативная регистрация inline (`server.py:922/944/946…`). **Спор:** нужен ли отдельный
  `config/ops/*.ops.yaml` (A3, F23-решение), или он должен ЗАМЕНИТЬ inline-вызовы (иначе два реестра
  операций — inline + yaml — разъедутся)? Пересмотреть F23 по факту кода: возможно A3 = «вынести
  аргументы register в yaml», а не «новый слой поверх».
- **OQ-B3 (два движка на одни `.xlsx`):** `table_engine` (read-model из `.read.json` + write-queue) и
  `excel_engine` (сырой openpyxl) работают по ОДНИМ книгам со стык-гэпом синхронизации (G16, xlsx↔read.json).
  **Спор:** это намеренный CQRS-раскол (чтение/запись) или случайное раздвоение ответственности? Где
  граница, кто источник правды для ячейки? Влияет на A1′/A7 (формулы уникальности читают чем?).
- **OQ-B4 (`_safe` широкий catch):** ловить типизированные исключения, не голый `ValueError` (F37) —
  мелко, но паттерн: реакция должна отражать реальную причину. Решить вместе с A6.
