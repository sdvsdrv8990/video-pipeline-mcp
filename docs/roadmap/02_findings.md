...
| F53 | 🟡 | **Тест-доки описывают несуществующее и противоречат директиве владельца.** (1) `tests/README.md`: в дереве `filesystem/`, `pipeline/`, `tables/` — таких каталогов нет; команда запуска `python3 tests/quick/test_import.py` — файла нет; требует «5 вопросов → в docstring» и формат `history.md` (отменено 2026-07-05: docstring в одну строку, рацио в commit, история git-native). (2) `tests/config/scenarios.yaml` предписывает регистрировать сценарии в `tests/runner.py` / `tests/scenarios/` + `SCENARIOS_MAP` — ничего из этого не существует (мёртвый конфиг, класс D2). (3) `CATALOG §E` рапортует `test_audit_fixes` 41/41 при факте 44/44. (4) память `project-rules §3.3` — та же отменённая «5 вопросов в docstring» | `ls tests/` vs дерево в README; `find tests -name runner.py -o -name scenarios` ∅; прогон = 44/44 | **I7 T3** |
