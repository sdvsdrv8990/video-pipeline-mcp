# СХЕМА КОНФИГА КАНАЛА (АРХИВ) — 7 секций переехали в листы `channel_data.xlsx`

> **📦 АРХИВНАЯ (S22, 2026-08-13).** Файла `config/channel_config.yaml` больше нет: 7 его секций
> перенесены механически (`scripts/config_to_schema.py`, round-trip проверен) в листы книги
> `channel_data` — одна книга на сущность (`13 §S20`). Документ сохранён, потому что несёт
> **РЕШЕНИЯ** (в конце файла), ради которых перенос и делался; они продублированы в шапке
> `config/templates/tables/channel_data.schema.yaml`. Схема-источник для конвертера — не этот файл.
> **Не путать** со снятой книгой `channel_config.xlsx`: та не существовала ни дня (`13 §ДВЕ сущности`).
>
