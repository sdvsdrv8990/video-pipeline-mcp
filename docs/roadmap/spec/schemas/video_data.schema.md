...
## Лист 24: `THUMBNAILS` 🆕 — журнал обложек и их периодов

> YouTube считает CTR на ВИДЕО, а не на обложку: «исторический CTR обложки» получить неоткуда, его
> приходится конструировать периодами. Строка = одна обложка со сроком, в который она стояла;
> `impressions` и `ctr_percent` вводит пользователь (интеграции с Analytics в MVP нет — решение 7).
> Без периодов сценарий «время от времени обложку лучше менять» измерить нечем.
> Уникальность обложки — ОТДЕЛЬНЫЙ балл: в `overall_uniqueness` он не входит, потому что тамошние
> веса уже дают ровно 1.00 и новое слагаемое сдвинуло бы числа во всех существующих каналах.
...
| `ctr_percent` | float | W | вводит пользователь |
| `thumbnail_uniqueness` | float | F 🆕 | отдельный балл, в `overall_uniqueness` НЕ входит |
| `notes` | string | W | |

---

## Лист 25: `THUMBNAIL_COMPONENTS` 🆕 — из чего собрана обложка

> Компоненты обложки берутся из тех, что УЖЕ использованы в видео: `global_asset_id` — ссылка на
> `ASSETS_USED`, а не второе описание того же стикмена. Один ассет, описанный дважды, разойдётся.
> Три идентификатора на строке (решение владельца): чья обложка, какого дизайна, какой компонент.
> Геометрия здесь по той же причине, что у `SCENE_ELEMENTS`: обложка — композиция, и положить
> компонент некуда, если негде хранить положение.
>
> **Использование ассета в обложке НЕ идёт в счётчики видео.** Уникальность обложки объявлена
> отдельным баллом (`THUMBNAILS.thumbnail_uniqueness`), и если бы `usage_count_in_video` рос от
> обложки, она влияла бы на уникальность видео через чёрный ход — вопреки тому же решению.
> Разведено структурно: счёт видео идёт по `SCENE_ELEMENTS`, счёт обложки — по этому листу.

| Столбец | Тип | Флаг | Прим. |
|---|---|---|---|
| `component_id` | string | id | идентификатор компонента в обложке |
| `thumbnail_id` | string | fk | → `THUMBNAILS` |
| `design_id` | string | fk | → `channel_data.THUMBNAIL_DESIGNS` |
| `global_asset_id` | string | fk | → `ASSETS_USED`: какой ассет видео переиспользован |
| `x` `y` | integer | W | положение в обложке |
| `width` `height` | integer | W | пусто = натуральный размер |
| `z_index` | integer | W | порядок наложения |
| `alpha_mode` | enum | W | AS_IS/REMOVE_BG/CHROMAKEY/NONE — как у элемента сцены |
| `notes` | string | W | |

---

## Лист 26: `POST_MORTEM` — журнал ошибок и уроков
...
## Лист 27: `ANALYTICS` — дашборд видео (Read-Only)
