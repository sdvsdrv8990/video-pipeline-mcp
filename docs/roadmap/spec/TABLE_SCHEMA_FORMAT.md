...
Разбирается **табличная** форма спеки (`| \`столбец\` | тип | флаг | прим. |`). НЕ разбирается и требует
ручной работы (конвертер сообщает поимённо, молча не теряет):
- **прозовая форма** — `\`a (id), b, c (fk)\` — \`W\`;` одной строкой (листы META/TRANSCRIPT/ANALYTICS и др.);
- **диапазоны** — `## Листы 4–10: \`ACT_1_HOOK … ACT_7_TRUTH\`` (7 листов одной структуры);
- **составные имена** — `VISUAL_/SCRIPT_/AUDIO_PATTERNS` (это несколько листов; `/` Excel запрещает);
