...
| IN6 | **Command injection** (если появится shell в media/ffmpeg) | `media_*` (**локальный инференс уже РЕАЛЬНЫЙ**: piper/diffusers/onnx через `core/runner`) | S24: команды строятся arg-списком, docker-порт только на loopback, токен не светится в `ps`; рой бьёт метасимволами (`atk_command_injection`) и проверяет отсутствие shell-sink в исходниках (`atk_shell_escape`) | остаётся риск при реализации ffmpeg-рендера | `execFile`/`subprocess` списком аргументов, без `shell=True`; allow-list кодеков из `render_config` | atk-inbound: cmd-injection (P-фаза) |
