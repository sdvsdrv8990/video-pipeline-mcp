...
Все 9 скилов ссылаются на удалённый `docs/dev/` (`workflow.md`×22, `threat_landscape`×9, `testing_strategy`×6, `audit/`×4, `history_*`). **Требуется** (skill-curator, отдельный шаг): заменить пути по таблице §1 — читать угрозы из `06`, стратегию тестов из `03`, findings/D#/G# из `02`, процесс из памяти `project-workflow-canonical`; write-back решений — в `_sessions.md`+commit, НЕ в `docs/dev/history_*`. До обновления скилы будут указывать в пустоту — это первый пункт Прохода-0.
