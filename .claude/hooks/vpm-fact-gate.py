...
ЧУЖАЯ = r"[^\x00-\x7F]"
SHELL_NON_ASCII = re.compile(
    rf"\$\{{?{ЧУЖАЯ}"                                            # $имя и ${имя}
    rf"|(?:^|[;&|(]|\b(?:export|declare|local|readonly|typeset)\s+)"
    rf"\s*{ИМЯ_ASCII}{ЧУЖАЯ}[^\s=]*="                            # присваивание в позиции имени
...
def pre(data: dict, st: dict, sid: str) -> None:
    ...
    if tool in ("Edit", "Write", "MultiEdit"):
        ...
        чужое = SHELL_NON_ASCII.search(
            SINGLE_QUOTED.sub("", INLINE_CODE.sub("", HEREDOC.sub("", raw))))
        if чужое:
