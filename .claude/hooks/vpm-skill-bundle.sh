...
# Роспись СТОРОЖЕЙ — та же болезнь, что была у скилов: они есть, но их не находят. Читается с
# диска и с их каталога зон, поэтому не может назвать несуществующего и не устаревает при
# переименовании. Владелец 2026-09-06: «их никто не читает, они толком не прижились, и если
# завтра попрошу улучшить — вероятность, что не найдёшь, очень высока».
зоны, команды = {}, {}
каталог = proj / "scripts/guards/CATALOG.md"
if каталог.exists():
    for имя, зона in re.findall(r"^\| `(\S+\.py)` \|([^|]*)\|", каталог.read_text(encoding="utf-8", errors="replace"), re.M):
        зоны[имя] = head(re.sub(r"[*`]", "", зона).strip(), 88)
роспись = proj / "scripts/guards/evidence.yaml"
if роспись.exists():
    текущий = None
    for строка in роспись.read_text(encoding="utf-8", errors="replace").splitlines():
        m = re.match(r"^  ([\w.-]+\.py):\s*$", строка)
        if m:
            текущий = m.group(1)
        elif текущий and (k := re.match(r'^\s+команда:\s*\[(.*)\]\s*$', строка)):
            команды[текущий] = " ".join(a.strip().strip('"\'') for a in k.group(1).split(",") if a.strip())
            текущий = None
сторожа = sorted(p.name for p in (proj / "scripts/guards").glob("*.py") if not p.name.startswith("_"))
роспись_строк = "\n".join(
    f"- `{имя}`{(' ' + команды[имя]) if команды.get(имя) else ''} — {зоны.get(имя, '⚠ зона не объявлена')}"
    for имя in сторожа)
источники = sorted(p.name for p in (proj / "scripts/guards").glob("_*.py"))
...
## Сторожа проекта: что лежит в `scripts/guards/` ПРЯМО СЕЙЧАС ({len(сторожа)})

{роспись_строк}

Источники улики (сами не судят, зовутся из сторожей): {", ".join(f"`{и}`" for и in источники)}.

Две команды, отвечающие «кого звать» и «что уже судится», — с них начинается работа со сторожами:
`patrol.py --задача <тип работы>` (улика · кто проснётся сам · кого поднять рукой · чем проверить
факт) и `invariants.py --приёмка` (карта: предмет × условие × состояние, и чего ещё нет).
Список прочитан с диска: сторожа, которого здесь нет, звать нельзя — его переименовали.
