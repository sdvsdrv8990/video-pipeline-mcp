...
def дом(каталог: Path | None = None) -> Path:
    """Путь разрешается в момент ВЫЗОВА: связанный дефолт подменить нельзя, и эмуляция со своим
    домом молча читала бы боевой след — на этом уже спотыкался словарь улик."""
    return каталог or Path.home() / ".claude" / "state" / "vpm-trace"
...
def mark(имя: str, каталог: Path | None = None) -> bool:
...
def след(каталог: Path | None = None) -> dict[str, dict]:
...
def seen(имя: str, каталог: Path | None = None) -> float:
    """Когда сторож срабатывал в последний раз. Ноль — не срабатывал ни разу."""
    return float(_прочесть(_файл(дом(каталог), имя)).get("last") or 0)


def рождение(каталог: Path | None = None) -> float:
    return float(_прочесть(_файл(дом(каталог), РОЖДЕНИЕ)).get("ts") or 0)


def dusty(имена: list[str], дней: float, каталог: Path | None = None) -> list[tuple[str, float]]:
    ...
    """
    сейчас, каталог = time.time(), дом(каталог)
