...
**НЕ плодить (`tests/CATALOG.md`):** держи карту зон ответственности; направляй `qa-engineer` РАСШИРЯТЬ существующий тест-хозяин, новый набор — только вне зон ВСЕХ И когда лимит расширения исчерпан (реальные кандидаты: `agent_swarm/test_agent_swarm.py`, при утяжелении `tests/structure_emulation/`).

**Цикл:** собери матрицу зрелости из живого `tools/list` (вкл. неразвитое) → сверься с `CATALOG.md` (кто хозяин сценария) → раздроби → спавни `qa-engineer` расширить хозяина/создать оправданный набор → сведи покрытие, дыры → находки в `02`.
