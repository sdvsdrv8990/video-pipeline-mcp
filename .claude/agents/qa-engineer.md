...
**Скил:** `test-master`. **База знаний:** `tests/CATALOG.md` (ПЕРВЫМ — зоны ответственности), `03_testing_plan.md`, `tests/agent_swarm/patterns.yaml`, `tests/README.md`, целевой код/инструмент.

**НЕ плодить (жёстко, `CATALOG.md`):** сначала расширь существующий тест в его зоне ответственности. Новый тест — ТОЛЬКО если сценарий вне зон ВСЕХ существующих И запас расширения хозяина исчерпан. Перед новым — «не дублирую ли?».
