"""
tests/bot_army/test_bot_army.py — adversarial: армия ботов против файрвола.

## Назначение
Массовое подключение с одного IP: rate limit, бан после порога нарушений, и то, что
соседний легитимный IP при этом проходит. In-process `Firewall`, без поднятия сервера.
"""
