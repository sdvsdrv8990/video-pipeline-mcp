...
## Зачем (T2, блокер F51)
Поведенческий тест обязан бить по живому серверу, а не по собранному в памяти движку: только так
проверяются транспорт, auth, файрвол и то, что сервер СООБЩАЕТ в консоль. Раньше такой тест
требовал заранее поднятого сервера на 8080 и потому жил вне гейта.

## Чужие данные не трогаем
Рабочая область — свежий временный каталог (`MCP_WORKSPACE`). Без этого набор правил бы настоящие
книги канала владельца, и «зелёный тест» стоил бы данных.

## Ключ настоящий
Сервер поднимается С аутентификацией: ключ передаётся через `MCP_AUTH_TOKEN`, и `.env` проекта не
трогается. Калитка `MCP_ALLOW_NO_AUTH` намеренно НЕ используется — иначе весь слой auth оставался
бы непроверенным, а именно его и надо проверить (F14).
"""
