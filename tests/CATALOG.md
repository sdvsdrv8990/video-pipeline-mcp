...
| `test_invariants.py` | **сам сторож `scripts/invariants.py`** на временных каталогах: каждая проверка ЛОВИТ своё нарушение и МОЛЧИТ на чистом | сторож без обеих сторон ослабляется незаметно | новые межфайловые инварианты | никогда — это единый дом проверок сторожа |
