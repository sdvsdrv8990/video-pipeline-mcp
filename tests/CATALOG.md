...
| `scenarios/` | **дом ОБЪЯВЛЕННЫХ сценариев и КАРТ** (`*.map.yaml`: состояния-предикаты, переходы, обход по всем переходам): `*.yaml` (`given/when/then/why`) + один исполнитель `tests/harness/scenario.py` против живого сервера. Сегодня зона — **E-D** (деструктив над таблицами: реакция, её класс и рецепт на проводе) | одинаковая обвязка «поднять → позвать → сверить» живёт в одном месте, а различия объявляются; журнал прогона на каждый шаг | E-E/E-F, любые сценарии «вызов → код реакции»; новые состояния и переходы карты (сценарии растут без кода); сложная подготовка — шагом в `steps.py` | — расширяем; новый ФАЙЛ сценариев = новая тема (свой сервер, своя область) |
