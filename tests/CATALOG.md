...
| `test_what_if.py` | **отчёт сторожа `scripts/guards/what_if.py`**: заявленное отделено от незаявленного, исчезнувшая проверка не выдаётся за «цвет не сменился», ПОЯВЛЕНИЕ и исчезновение объявляемы наравне со сменой цвета (у одного имени исходов бывает несколько — вставка шага рождает новые проверки и уносит старые под новым номером), роспись трёх карт полна (у каждого сторожа есть набор-дом) и одна метка на несколько проверок не съедает соседей | сравнение двух деревьев стоит минуты и в быстрый набор не влезает — суть проверяется подставленными вердиктами | новые роды исходов сравнения (появилась/исчезла/сменила цвет) | никогда — это единый дом отчёта сравнения |
