...
| `test_trail.py` | **запись и её потребитель**: писатель следа (`core/observability` — удержание файлов, предел, неполное объявление, отказ записи) И синтезатор (`scripts/guards/reproduce.py` — отбор отказа, форма порождённого объявления) | обе половины читают ОДНУ форму записи: разъехавшийся формат ломает их сразу, а порознь этого не видно | второй источник записи (протокольные отказы файрвола), повышение последовательности до карты | никогда — это единый дом записи; поведение «отказ попал в след» живёт в сценариях `observability.yaml`, а не здесь |
