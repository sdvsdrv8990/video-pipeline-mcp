...
| M44 | проверка СОДЕРЖИМОГО снята (скрипт под видом .md проходит) | `core/write_policy.py` | ⬜ **тест не написан** (лимит сессии S19) | — |
| M45 | `fs_rename`/`fs_move` не проверяют тип цели (обход allowlist) | `tools/filesystem/__init__.py` | ⬜ **тест не написан** (лимит сессии S19) | — |
