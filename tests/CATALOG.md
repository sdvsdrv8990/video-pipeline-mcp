...
| `test_what_if.py` | **отчёт сторожа `scripts/guards/what_if.py`**: заявленное отделено от незаявленного, а исчезнувшая проверка не выдаётся за «цвет не сменился» | сравнение двух деревьев стоит минуты и в быстрый набор не влезает — суть проверяется подставленными вердиктами | новые роды исходов сравнения (появилась/исчезла/сменила цвет) | никогда — это единый дом отчёта сравнения |
