...
| `test_trail.py` | **писатель следа вызовов** (`core/observability`): удержание файлов, предел на файл, неполное объявление, отказ записи | след — единственное, из чего воспроизводят отказ живой сессии; сломанный след обязан терять НАБЛЮДЕНИЕ, а не вызов | синтезатор сценария из следа, второй источник записи (протокольные отказы файрвола) | никогда — это единый дом следа; поведение «отказ попал в след» живёт в сценариях `observability.yaml`, а не здесь |
