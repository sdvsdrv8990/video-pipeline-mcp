...
| `scenarios/` | **дом ОБЪЯВЛЕННЫХ сценариев и КАРТ** (`*.map.yaml`: состояния-предикаты, переходы, обход по всем переходам): `*.yaml` (`given/when/then/why`) + один исполнитель `tests/harness/scenario.py` против живого сервера. Зоны сегодня — **вся опись инструментов по проводу (72 из 72)**: E-D деструктив над таблицами, провенанс вывода, живучесть, файловые операции, отложенная запись и очередь (`tables_ops`), правки книг (`excel_ops`), осмотр данных (`data_inspection`), поиск (`search` — три пресета одного `QueryPlan` различаются проводкой, а не движком), структура и память (`structure_ops`), провайдеры (`media_ops` — контракт отказа вместо весов) монтаж (`montage_ops` — цепочка отказов называет недостающее) и **периметр** (`perimeter.yaml` — ключ, манифест и протокольный отказ; шаг объявляет `rpc`/`headers`/`token`, ожидание — `http`) (`filesystem.yaml` — запись ЧЕРЕЗ инструмент, а не фикстурой: иначе путь записи не исполняется вовсе, F141) | одинаковая обвязка «поднять → позвать → сверить» живёт в одном месте, а различия объявляются; журнал прогона на каждый шаг | E-E/E-F, любые сценарии «вызов → код реакции»; новые состояния и переходы карты (сценарии растут без кода); сложная подготовка — шагом в `steps.py` | — расширяем; новый ФАЙЛ сценариев = новая тема (свой сервер, своя область) |
