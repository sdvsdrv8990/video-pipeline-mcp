...
| `scenarios/` | **дом ОБЪЯВЛЕННЫХ сценариев и КАРТ** (`*.map.yaml`: состояния-предикаты, переходы, обход по всем переходам): `*.yaml` (`given/when/then/why`) + один исполнитель `tests/harness/scenario.py` против живого сервера. Зоны сегодня: **E-D** (деструктив над таблицами: реакция, её класс и рецепт на проводе), провенанс вывода, живучесть, **поиск через провод** (`search.yaml` — три пресета одного `QueryPlan` различаются проводкой, а не движком), **осмотр данных** (`data_inspection.yaml`) и **файловые инструменты** (`filesystem.yaml` — запись ЧЕРЕЗ инструмент, а не фикстурой: иначе путь записи не исполняется вовсе, F141) | одинаковая обвязка «поднять → позвать → сверить» живёт в одном месте, а различия объявляются; журнал прогона на каждый шаг | E-E/E-F, любые сценарии «вызов → код реакции»; новые состояния и переходы карты (сценарии растут без кода); сложная подготовка — шагом в `steps.py` | — расширяем; новый ФАЙЛ сценариев = новая тема (свой сервер, своя область) |
...
| **F3** | провайдеры = честные стабы (G16) | behavioral | **хозяина нет** | ⬜ НЕ покрыт: прежний хозяин `render_draft_final` зовёт ноль провайдерских инструментов — покрытие числилось, но его не было |
