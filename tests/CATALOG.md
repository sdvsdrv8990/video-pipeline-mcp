...
| `test_comment_guard.py` | **сам гейт текста `scripts/comment_guard.py`**: что обязан ловить и что обязан пропускать | храповик, который перестал ловить, выглядит как зелёный | новые классы текста в коде | никогда — это единый дом гейта текста |
...
| M124 | зона набора ищется по всему тексту каталога, а не первой ячейкой строки таблицы | `scripts/invariants.py` | `test_invariants` | 🔴 ловит 1 ассерт (S25) — упоминание в прозе снова засчитывалось бы зоной |
...
рядом с зонами, а проверяется сторожем (`scripts/invariants.py`) по мере `T11`.
