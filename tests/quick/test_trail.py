...
# Правило стоит только у роли УЛИКА: отказ и вердикт повторяются своим прогоном, а не
# строкой, и распространение правила на них сломало бы обоих живых производителей.
try:
    _stamp.sign("проба", "УЛИКА", "проза вместо команды", cmd="замер по regex", root=tree)
    ok(False, "проза в поле «повторить» ОТВЕРГНУТА")
except ValueError as _беда:
    ok("поднимать нечем" in str(_беда), "проза в поле «повторить» отвергнута с причиной")
ok(_stamp.sign("гейт", "ОТКАЗ", "прозу отказу можно", cmd="см. текст отказа", root=tree)[0],
   "правило только для улики — отказ гейта прозу принимает и не сломан")
