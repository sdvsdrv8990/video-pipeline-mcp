"""
tests/quick/test_audit_fixes.py — Регрессия по исправлениям аудита (D1–D13)

## Что тестируем
Эмпирически подтверждаем, что дефекты аудита закрыты и не вернулись:
...
## Зачем нужен
Эти дефекты были доказаны исполнением кода в аудите (docs/dev/audit/v2/AUDIT.md).
Тест фиксирует их как регрессию: если кто-то откатит фикс — тест покраснеет.
