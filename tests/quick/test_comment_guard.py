...
# Комментарий, называющий то же, что следующая строка кода: его цена нулевая.
_эхо = "def f():\n    # строки файла\n    строки_файла = 1\n    return строки_файла\n"
ok("пересказ следующей строки пойман",
   any("пересказывает следующую строку" in x for x in notes_of(_эхо)), notes_of(_эхо))
_польза = "def f():\n    # ключ усечён до восьми: коллизии считает соседний инвариант\n    имя = 1\n    return имя\n"
ok("комментарий, добавляющий НЕОЧЕВИДНОЕ, замечанием не стал", notes_of(_польза) == [],
   notes_of(_польза))
_шебанг = "#!/usr/bin/env python3\n\"\"\"проба.py — роль.\"\"\"\n"
ok("шебанг пересказом не считается", notes_of(_шебанг) == [], notes_of(_шебанг))
_метка = "def f():\n    # ── Excel ──\n    excel_create(1)\n"
ok("одинокий разделитель — метка навигации, а не пересказ", notes_of(_метка) == [],
   notes_of(_метка))
