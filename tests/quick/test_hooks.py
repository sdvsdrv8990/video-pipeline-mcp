...
def main() -> int:
    ...
    # Третья форма данных — ОДИНАРНЫЕ кавычки: подстановки там нет, значит это текст. В двойных
    # подстановка есть, и они судятся по-прежнему. Поймано на ложном отказе шаблону `grep`.
    ok(not any(ловится(к) for к in ("grep -aE 'Туннель|Статус' log", "echo 'дом=1 и прочее'",
                                    "sed -n '/остаток/p' file")),
       "одинарные кавычки — данные: шаблон и текст с «=» внутри них не обвиняются")
    ok(ловится('ls -d "$ДОМ"') and ловится("К=/tmp; echo"),
       "в двойных кавычках подстановка есть — там правило судит как прежде")
