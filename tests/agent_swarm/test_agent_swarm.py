"""
tests/agent_swarm/test_agent_swarm.py — рой агентов против ЖИВОГО сервера.

## Назначение
Гоняет реестр `patterns.yaml` по серверу, который поднимает себе сам (харнесс T2).
Standalone-прогон: `python tests/agent_swarm/test_agent_swarm.py`.

## Границы
- Реестр и код сходятся в обе стороны: паттерн без исполнителя и исполнитель без паттерна
  одинаково красят гейт; отложенные перечислены явно и пройденными не считаются.
- У `xfail-spec` красит гейт и СРАБОТАВШАЯ защита — значит статус в реестре устарел.
- Фазы разнесены по процессам: файрвол считает частоту по IP, а рой весь с 127.0.0.1 —
  один длинный прогон забанил бы сам себя и блокировки шли бы по ложной причине.
"""
import json
import socket
import sys
...
def atk_anomaly_sequence(c: Ctx):
    """Правило аномалий обязано быть ПОДКЛЮЧЕНО — след в консоли сервера, а не в конфиге."""
...
def atk_weaponized_destruct(c: Ctx):
    """Обманутая модель бьёт по данным клиента — спасает containment на КАЖДОЙ операции.
