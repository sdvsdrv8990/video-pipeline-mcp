...
## Где лежат и почему там
Ключ — свойство КАНАЛА, а не сервера: у разных каналов разные счета и разные поставщики.
Поэтому файл ключей лежит в самом канале, рядом с его данными: `<канал>/.secrets/provider_keys.enc`.
Каталог `.secrets` объявлен закрытым в единой точке путей (`core/paths`), поэтому ни один
инструмент до него не дотягивается — ни чтением, ни листингом, ни поиском, ни записью.

## Значение не появляется на экране НИКОГДА
Ключ шифруется в момент ввода, до записи на диск (`scripts/set_provider_key.py`, ввод скрыт).
Наружу — только отпечаток (`sha256`, первые байты) и дата: по нему видно, тот ли ключ стоит,
и невозможно восстановить сам ключ. Модель не видит значения даже при захвате экрана владельца.

## Что эта защита НЕ даёт (честная граница)
Шифрование на диске защищает от инструментов сервера и от чужих глаз в кадре, но не от того,
у кого есть shell на этой машине: ключ шифрования лежит рядом (`instance_secret.key`, `0600`,
вне рабочей области и вне git). Модель, работающая через MCP, shell не имеет — для неё граница
жёсткая. Владелец, у которого shell есть, и так владеет ключами.

## Ротация
Новый ключ ЗАМЕНЯЕТ старый в той же строке провайдера: два действующих ключа на одного
поставщика — источник вопроса «каким сейчас платят», а на него не должно быть двух ответов.
"""
