...
## Назначение
Каталог шаблонов был зашит на месте вызова, поэтому режим «свой шаблон под проект» был
невозможен без правки кода. Резолвер отвечает на вопрос по АДРЕСУ создания: есть ли выше по
дереву каталог `.templates/` — берём его, нет — серверный `config/templates/`.

## Почему по адресу, а не по состоянию
Состояние «проект открыт» на сервере завести нечем и не нужно: адрес создания известен всегда,
а владелец выводится из него же (`ChainResolver`). Резолвер без памяти — два одинаковых вызова
дают одинаковый ответ.

## Containment
Поиск идёт ТОЛЬКО внутри `workspace/` и только вверх до его корня (G17): шаблон проекта — это
данные, и выйти за рабочую область он не может по построению. Материализация по найденным
шаблонам всё равно идёт через `write_policy` (F72/F73) — резолвер не даёт послаблений.
"""
