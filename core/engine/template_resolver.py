...
## Границы
Поиск идёт ТОЛЬКО внутри `workspace/` и только вверх до его корня. Материализация по
найденным шаблонам всё равно идёт через `write_policy` — послаблений резолвер не даёт.
"""
