...
## Назначение
Всё, что сервер пишет в `workspace/`, помечается двумя признаками: **подписью** этого инстанса
(доказывает авторство) и **отпечатком машины** (доказывает, что писали здесь). Запись без подписи
или с чужой подписью не применяется — так второй такой же сервер, другой агент или ручная правка
файла перестают быть невидимыми.

## Решения (`15 §3-тер`, разбор варианта владельца)
- **Ключ ≠ железо.** Аппаратные ID не секреты (их читает любой локальный процесс), а `product_uuid`
  и `board_serial` требуют root — это конфликт с инвариантом «сервер не root» (F35). Поэтому
  авторство доказывает ключ Ed25519, а `machine-id` идёт **рядом** как отпечаток окружения.
- **Не кирпич.** Несовпадение отпечатка (смена железа, переезд VM, восстановление из бэкапа)
  делает workspace read-only и требует явного `--re-adopt`, а не запирает владельца в его данных.
- **Без своей криптографии.** Подпись — Ed25519 из `cryptography`; при отсутствии библиотеки
  подпись честно недоступна (G16: не подменяем «самодельным хешем», который выглядит как защита).
