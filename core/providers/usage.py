...
## Зачем
Лимиты и fallback в резолвере — теория, пока `current_usage` стоит на месте: «исчерпан» не
наступит никогда, и запасной провайдер не включится ни разу. Здесь расход становится фактом.

## Чем меряется
Единица объявлена в строке провайдера (`usage_unit`): вызов, символ, файл. Способов ИЗМЕРИТЬ
ровно три и они в коде; какой единицей меряется конкретный провайдер — данные канала.
Неизвестная единица не считается молча нулём — это отказ: тихий ноль означал бы вечный лимит.

## Почему счётчик пишется мимо очереди
Очередь `write.json` — дверь для правок ИИ: они копятся и применяются осознанным
`json_execute_queue`. Расход туда класть нельзя дважды: он не двигался бы до чужого вызова,
а применение утащило бы вместе с ним ВСЕ накопленные чужие операции — молчаливый побочный
эффект. Поэтому счётчик пишется точечно в снапшот (containment у `StateManager` тот же).
"""
