...
## Назначение
`sync_mode: false` означает, что провайдер отвечает идентификатором задачи, а не результатом:
trigger → poll → download. Этот модуль реализует ожидание и проверку того, что файл ДЕЙСТВИТЕЛЬНО
лёг на диск. Как ждать и что считать готовым — `config/media_tasks.yaml`, не код.

## Зачем цикл на самом деле
Не «дождаться готового», а ПОЙМАТЬ ОТКАЗ. Онлайн-модель заворачивает запрос по десятку причин
(модерация, лимит провайдера, недоступность, невалидный вход); без прозвонки это молчание.
Цикл достаёт причину и отдаёт её ИИ — анализировать и решать, что делать.

## Это не планировщик
Демона на сервере нет (решение владельца S15): цикл — часть вызова, в фоне ничего не «тикает».
Отсюда честная граница: отвалившийся клиент задачу НЕ теряет и НЕ спасает — она живёт на стороне
провайдера, и её статус прозванивается заново.

## Провайдер-агностично
Модуль не знает ни одного провайдера: опрос передаётся вызывающим как функция, имена статусов
берутся из декларации. Новый провайдер = новые слова в YAML.

## Почему проверка загрузки отдельная
Ответ «200 OK» не означает, что файл на диске: бывает нулевой размер, обрыв на середине, путь
вне рабочей области. Поэтому факт загрузки проверяется по диску, а не по коду ответа.
"""
