...
## Назначение
Строку канала заполняет ИИ, а пределы модели знает только сама модель: без спеки параметры
пишутся вслепую и невозможное всплывает платным отказом — или тихо не тем результатом.

## Границы
- формат — JSON Schema (язык `input_schema` инструментов), проверяет её установленная
  библиотека; наше здесь — только политика: откуда взять и что считать расхождением;
- источники по порядку: провайдер → `config/model_specs.yaml` → файлы модели на диске;
  что нашлось, называется в ответе;
- спеки нет — так и говорим: «не знаем» не выдаётся за «ограничений нет».
"""
