...
## Зачем цикл, а не «поставил и вернул»
Веса — гигабайты: установка длится минуты, а то и десятки минут, и рвётся на середине (сеть,
место на диске, отказ источника). Один синхронный вызов на всё это либо упрётся в таймаут
клиента, либо вернёт молчание, из которого не видно, идёт работа или уже умерла. Поэтому
установка запускается, а её ход прозванивается — тем же приёмом, что и задача провайдера.

## Прогресс меряется ПО ДИСКУ
Не по проценту, который сообщил бы загрузчик, а по тому, сколько байт реально легло в каталог.
Загрузчик может отчитаться и оборваться; диск не врёт. Отсюда же признак «зависло»: байты не
растут дольше объявленного срока — состояние `stale`, а не вечное «идёт».

## Это не планировщик
Поток живёт РОВНО столько, сколько идёт эта установка, и умирает вместе с ней; между запросами
клиента ничего не «тикает» (решение владельца S15 в силе). Отвалившийся клиент установку не
теряет и не спасает — она продолжается, и её состояние прозванивается заново.

## Повтор дешёвый
Прерванная загрузка возобновляется с места обрыва (кэш загрузчика), поэтому «повторить» —
нормальный ответ на сбой, а не «качать всё заново».
"""
