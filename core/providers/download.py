...
## Назначение
Сама загрузка результата (приёмка «файл действительно лёг?» — в `task_cycle.verify_download`).
Ссылку выдаёт провайдер, то есть она приходит ИЗВНЕ и проверяется как любой чужой ввод.

## Границы
- схема и адрес проверяются ДО запроса (`config/media_tasks.yaml → download.fetch`): иначе
  ссылка на `169.254.169.254`/`localhost` уводит сервер туда, куда клиент не достаёт (SSRF);
- предел размера считается по ходу потока — узнать о гигабайтах после записи поздно;
  превышение обрывает загрузку и удаляет недописанный файл.
"""
