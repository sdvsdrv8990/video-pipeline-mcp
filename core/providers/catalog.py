...
## Назначение
Список моделей берётся ЖИВЫМ из каталогов (индекс голосов piper, Hugging Face, реестр LiteLLM);
здесь — только правила отбора и установки.

## Границы
- отсев объявлен в `config/providers.yaml → local.sources.*.filters` — меняется без правки кода;
- онлайн-список отвечает офлайн, из реестра LiteLLM: ключ нужен для ВЫЗОВА, не для перечисления;
- запрет на pickle-форматы весов (`local.install.deny_suffixes`) — не вкусовщина: их загрузка
