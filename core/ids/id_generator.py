...
class IDGenerator:
    ...
    Единственный источник — `core.ids.Taxonomy` (блок `id:` в `*.tpl.yaml`
    ...
    def _generate_unique(self) -> str:
        ...
        """
        # НЕ усекаем хеш до 8 hex (32 бита) — по парадоксу дней рождения
    ...
    def is_valid_format(self, entity_id: str) -> bool:
        ...
        if len(parts) != 2:
            return False
        prefix, unique = parts
        # Уникальная часть теперь uuid4.hex (32 hex-символа).
