...
## Назначение
Транспорт вокруг уже работающих адаптеров: `/run` принимает задачу от сервера, отдаёт её
существующему локальному адаптеру (`onnx_bg`, `onnx_upscale`, `piper_local`, `diffusers_local`)
через тот же пул моделей и отвечает путём к готовому файлу. Своей логики инференса здесь нет.

## Почему это не демон (S15)
Раннер ничего не подхватывает и не тикает: он поднимается явной командой (`media_runner`) и
отвечает на запросы. Между запросами он не делает НИЧЕГО, кроме того что держит поднятые модели.
