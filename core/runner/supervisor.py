...
## Назначение
pid, порт и токен раннера лежат в файле (только для владельца), а живость проверяется прозвонкой,
а не верой в запись: сервер перезапустили, а раннер жив — иначе второй запуск поднял бы второй
процесс на занятый порт.

## Границы
Авто-перезапуска здесь НЕТ намеренно: в отличие от туннеля раннер падает по причине (OOM, битые
веса, отсутствующая зависимость), и тихо поднять его заново значит спрятать причину и уронить
снова. Упал → это видно как `exited`, а поднимает человек или ИИ той же командой.
"""
