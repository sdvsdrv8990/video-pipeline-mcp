...
### 1. Код
- RateLimiter класс с методом check(ip, timestamp)
- Хранит историю запросов по IP
- Возвращает allowed: bool

### 2. Поведение
- Считает запросы за последнюю минуту
- Если превышен лимит → не пропускает
- Автоматический бан после N нарушений

### 3. Поток данных
```
Запрос → RateLimiter.check(ip, timestamp)
   ├── allowed=True → пропускаем
   └── allowed=False → бан + ErrorDetail
```

### 4. Долгосрочный (6 мес)
- Лимиты адаптируются к нагрузке
- Статистика помогает оптимизировать

## Порядок полей
1. Пороги (max_requests, ban_after)
2. Состояние (requests, violations)
