"""
core/declaration.py — единственная дверь к декларациям config/*.yaml.

## Назначение
Одна загрузка YAML на весь сервер: провайдеры, монтаж, файрвол и транспорт читают свои
декларации одинаково — иначе копии расходятся по мелочам (одна перечитывает, другая нет,
у третьей отсутствие файла означает «правило выключено»).
...
class Declaration:
    ...
    def optional(self) -> dict:
        """Декларация, отсутствие которой — решение вызывающего, а не отказ.

        Битость отказ ВСЕГДА: непрочитанный файл нечем отличить от пустого, и правило,
        собранное из пустоты, тихо перестаёт работать.
        """
        return self.data if self.config_file.exists() else {}
