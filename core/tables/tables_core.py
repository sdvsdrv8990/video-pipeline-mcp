"""core/tables/tables_core.py — generic-движок табличных данных.
...
через очередь (`QUEUEABLE_ACTIONS`), чтения в неё не кладутся. Запись в `computed`-столбец
отбивается кодом COMPUTED_READONLY, значение вне `schema.enum` — ENUM_VIOLATION: сервер
пишет в обход дропдауна, поэтому проверка живёт здесь. Состояния между вызовами ядро
не помнит, факты возвращает вызывающему.
"""
