...
Поэтому конвертация механическая (обход структуры), а не разбор прозы — в отличие от
`spec_to_schema.py`, который читает markdown-спеки.

Приёмка сильнее, чем у `spec_to_schema`: обратная сборка из ТЕКСТА полученных листов обязана
дать исходные секции (`--verify`). Не сошлось → конвертация теряет данные, переносить нечего.

Первое применение — конфиг канала → `channel_data` (S22, `13 §S20`); тем же механизмом
переносится конфиг конкурента, когда он появится (`13 §S20 шаг 5`).
...
# Список ключей — из плана `13 §S20 шаг 2`, а не догадка по данным.
...
# из плана `13 §S20 шаг 2`: расширять его опасно. `day_of_week` с двумя встреченными днями
