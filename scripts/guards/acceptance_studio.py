...
def styles(got: dict, tree: Path = TREE, **_) -> list[str]:
    ...
    for name in sorted(set(прежние) | set(нынешние)):
        ...
        for поле in разошлось:
            # У композиции своя формулировка: «рисует» отвечает на вопрос владельца «попал ли
            # компонент туда, куда ожидалось» — переезд примитива к другому родителю виден здесь
            # раньше, чем на экране.
            если = ("оказался не там, где был: рисует" if поле == "рисует"
                    else f"{поле} разошлось со снимком")
            notes.append(f"{name}: {если} — было {было.get(поле)!r}, "
