...
Комментарий объясняет НЕОЧЕВИДНОЕ поведение, терсово; разбор задачи, история и дубль декларации —
в журнал и commit. Сюда же ЯРЛЫКИ: литерал прямым аргументом своей же функции — тот же текст в
коде, только напечатанный человеку в отчёт.

## Зона: ДЕРЕВО, а не событие
Судит дерево целиком, механически, без вопросов к автору — `pre-commit`, джоба CI, храповик.
Смежник `.claude/hooks/vpm-comment-intent.py` судит СОБЫТИЕ и спрашивает о ЦЕНЕ, содержимого не
читая. Замер: на образце здесь 4 находки, там ни одной — мерить это заново не нужно.

Режимы: `--hook` · `--scan` · `--check` (exit 1) · `--bless`. Границы текста даёт `ast`+`tokenize`,
в прочих языках `pygments`, без лексера — по решётке. Цели: `git ls-files` ВМЕСТЕ с
неотслеживаемым, иначе новый файл не судится вовсе.
"""
