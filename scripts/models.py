...
## Назначение
Список берётся ЖИВЫМ (реестр шлюза, индекс голосов piper, каталог Hugging Face); что считать
лишним, объявлено в `config/providers.yaml → local.sources.*.filters`, поэтому отсев правится
данными. Онлайн-список виден без ключей: ключ нужен для вызова (`scripts/set_provider_key.py`).

## Границы
Ставим только форматы, которые не являются кодом: `.bin/.ckpt/.pt` — pickle, его загрузка
исполняет код из файла. Опись `vendor/models/installed.yaml` дополняется, но не переписывается:
это рукописный конфиг, и первая же машинная запись стёрла бы объяснения владельца.
"""
