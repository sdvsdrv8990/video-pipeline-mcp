...
├── pyproject.toml         ✅ сборка/зависимости (PEP 621) — единственный файл зависимостей
...
./run.sh --no-auth    # цикл разработки: коннектор подцепляется без ключа, сервер об этом кричит
```

## Зависимости

Единственный источник — `pyproject.toml` (`pip install -e .`);
dev-инструменты — `pip install -e ".[dev]"` (агрегат зон `test`/`scripts`/линт). Отдельного lock-файла нет: из него никто не
ставил, а расходился он молча.
...
Гейт — семь джоб `ci.yml`, и «зелёно» считается по exit-коду каждой, а не по одному `pytest`:
