...
## Назначение
`ToolContext` раздаёт группам движки (данные/структура/шаблоны/связи) и хелперы
`resolve`/`err`/`safe`; `build_context` собирает его из `Engine`/`IDGenerator`/`StateManager`;
